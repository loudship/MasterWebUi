...
def encrypt_payload(payload_dict: list | dict) -> bytes:
    """Encrypts a Python dict/list with Fernet (AES-128-CBC + HMAC-SHA256)."""
    json_str = json.dumps(payload_dict)
    return fernet.encrypt(json_str.encode("utf-8"))

def decrypt_payload(encrypted_blob: bytes) -> list | dict:
    """Decrypts a Fernet blob back into a Python dict/list."""
...
def save_credentials(domain_id: str, payload: list | dict):
    # try/finally: an exception mid-transaction must not leak the pooled
    # connection (sessions previously stayed open on any error path).
    db = SessionLocal()
    try:
