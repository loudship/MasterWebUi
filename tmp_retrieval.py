...
async def get_rag_config(request: Request, user=Depends(get_admin_user)):
    return {
        ...
        'web': {
            ...
            'YOUTUBE_LOADER_TRANSLATION': request.app.state.config.YOUTUBE_LOADER_TRANSLATION,
...
def save_docs_to_vector_db(
    ...
    # --- PHASE 7 NER EXTRACTION INJECTION ---
    import re
    
    # Comprehensive stopword filter
    ENTITY_STOPWORDS = {
    ...
    def extract_entities_cpu(text):
        ...
        # Extract proper nouns with improved filtering
        # Avoid capturing sentence starts using negative lookbehind
        propn = re.findall(r'(?<![.!?]\s)(?<![.!?\n]\s)\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b', text)
        for term in propn:
            # Skip stopwords and very short terms
            if term not in ENTITY_STOPWORDS and len(term) > 2 and term.lower() not in seen:
    ...
    for i in range(len(docs)):
        docs[i].metadata["entities"] = extract_entities_cpu(texts[i])
    # --- END PHASE 7 NER EXTRACTION INJECTION ---
...
async def delete_entries_from_collection(
    ...
    try:
        if await ASYNC_VECTOR_DB_CLIENT.has_collection(collection_name=form_data.collection_name):
            ...
            result = await ASYNC_VECTOR_DB_CLIENT.delete(
                collection_name=form_data.collection_name,
                filter={'hash': hash},
            )
